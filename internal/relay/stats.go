...
type pathStats struct {
	...
	// Bytes arriving over the last second, in two half-second buckets, for
	// shortRxKbps (the RxKbpsBy16 wire report). The thirty-second window
	// below is right for judging a path and far too slow for that.
...
// shortWindowBucket is half the short byte-rate window used by shortRxKbps.
const shortWindowBucket = 500 * time.Millisecond

// rollShortBytes turns the short-window byte bucket over, for shortRxKbps.
func (s *pathStats) rollShortBytes(now time.Duration) {
	if now-s.shortStart < shortWindowBucket {
		return
	}
	if now-s.shortStart < 2*shortWindowBucket {
		s.shortPrevBytes = s.shortBytes
...
func (s *pathStats) shortRxKbps(now time.Duration) float64 {
	...
	if cur >= 2*shortWindowBucket || cur < 0 {
		return 0
	}
	bytes, span := s.shortBytes, cur
	if s.shortPrevValid {
		bytes += s.shortPrevBytes
		span += shortWindowBucket
	}
